# B4 (Beam4K) Project Dashboard

**Nexar's Next-Generation 4K Smart Dashcam**

| Phase | Status |
|-------|--------|
| **Field Test 3 / MVP Development** | ![In Progress](https://img.shields.io/badge/In_Progress-blue) |
| **Blocker** | PVT sign-off waiting on Chicony samples |

---

## Quick Links

| | Link |
|---|------|
| **Timeline** | [Project Timeline](https://getnexar.atlassian.net/jira/software/c/projects/FS/boards/268/timeline?statuses=2%2C4&timeline=MONTHS) |
| **Releases** | [JIRA Releases](https://getnexar.atlassian.net/projects/FS?selectedItem=com.atlassian.jira.jira-projects-plugin%3Arelease-page&status=no-filter) |
| **Release Plan** | [Confluence](https://getnexar.atlassian.net/wiki/spaces/EMB/pages/4832722963) |
| **JIRA Internal** | [FS Board](https://getnexar.atlassian.net/jira/software/c/projects/FS/boards/268/backlog) · [BR Bugs](https://getnexar.atlassian.net/jira/software/c/projects/BR/boards/287/backlog?issueParent=109691) |
| **JIRA External** | [B4 (Chicony)](https://nexar-chicony.atlassian.net/jira/core/projects/B4/board?filter=&groupBy=status) · [DAS (Dastic)](https://nexar-chicony.atlassian.net/jira/core/projects/DAS/board?filter=&groupBy=status) |
| **Sprint Planning** | [WW49 Planning](https://getnexar.atlassian.net/wiki/spaces/EMB/pages/5111021619) |
| **Serial Numbers** | [FT Tracking Sheet](https://docs.google.com/spreadsheets/d/1ZAwoMznI-whqYJFvrwy9SrFTTNGQiMq62E_86qvR_sw/edit?gid=243956152#gid=243956152) |
| **ODM Export** | [Google Drive](https://drive.google.com/drive/folders/1lFlqGslitGcLlwvC3xXvD4WrOqcWQ6Fh) |
| **Slack** | [#eng-beam4k](https://app.slack.com/client/T02KEL8KX/C0824FCA2GM) · [#general-beam4k](https://app.slack.com/client/T02KEL8KX/C08M9J1S9CG) |
| **Jenkins** | [B4 Builds](https://ci.nexar.cloud/job/Firmware/job/build-nexar-chicony/job/b4hw-fw2/) |

---

## Releases

| Version | Status | Date | Notes |
|---------|--------|------|-------|
| fw2-b4-v7.4.60 (MVP1) | ![Unreleased](https://img.shields.io/badge/Unreleased-blue) | 2025-12-15 | |
| fw2-b4-v7.4.53 (FT-3) | ![Released](https://img.shields.io/badge/Released-green) | 2025-12-07 | Initial MVP version |
| fw2-b4-v7.4.51 | ![Released](https://img.shields.io/badge/Released-green) | 2025-11-24 | |
| fw2-b4-v7.4.50 (FT-2) | ![Released](https://img.shields.io/badge/Released-green) | 2025-11-20 | Complete Phases 1 & 2 |
| fw2-b4-v7.3.51 (FT-1) | ![Released](https://img.shields.io/badge/Released-green) | 2025-11-11 | FT version (patch) |
| fw2-b4-v7.3.50 | ![Released](https://img.shields.io/badge/Released-green) | 2025-11-10 | FT version (partition B) |
| fw2-b4-v1.0.2 | ![Released](https://img.shields.io/badge/Released-green) | 2025-10-29 | PVT 1.0 Patch |
| fw2-b4-v1.0.1 | ![Released](https://img.shields.io/badge/Released-green) | 2025-10-20 | Fix version update |
| fw2-b4-1.0.0 | ![Released](https://img.shields.io/badge/Released-green) | 2025-10-19 | PVT (Partition A) |
| B4 DVT sign-off | ![Released](https://img.shields.io/badge/Released-green) | 2025-10-02 | |

---

## Milestones

| Milestone | Status | Date |
|-----------|--------|------|
| DVT signoff | ![Done](https://img.shields.io/badge/Done-green) | 2024-Q3 |
| PVT 0.5 | ![Done](https://img.shields.io/badge/Done-green) | 2024-Q4 |
| PVT 1.0 | ![Done](https://img.shields.io/badge/Done-green) | 2024-Q4 |
| Initial FT | ![Done](https://img.shields.io/badge/Done-green) | Oct 2024 |
| Field Test 2 | ![Done](https://img.shields.io/badge/Done-green) | Nov 2024 |
| Field Test 3 | ![Active](https://img.shields.io/badge/Active-blue) | Dec 2024 |
| PVT sign-off | ![Blocked](https://img.shields.io/badge/Blocked-red) | TBD |
| MVP | ![Active](https://img.shields.io/badge/Active-blue) | Q1 2025 |
| Full Product | ![Backlog](https://img.shields.io/badge/Backlog-lightgrey) | TBD |

---

## Top 5 Priorities (WW51)

| # | Priority | Ticket |
|---|----------|--------|
| 1 | FT Readiness: OBD issue | TBD |
| 2 | MVP: Remote stream bugs | [FS-3375](https://getnexar.atlassian.net/browse/FS-3375) |
| 3 | MVP: Encryption | [FS-3283](https://getnexar.atlassian.net/browse/FS-3283) |
| 4 | MVP: HTTP Server "Teepee" | [FS-2677](https://getnexar.atlassian.net/browse/FS-2677) |
| 5 | Integrate new Logstore | [FS-3322](https://getnexar.atlassian.net/browse/FS-3322) |

---

## Metrics

| Metric | Count |
|--------|-------|
| Open Bugs | 16 |
| FS Tickets | 40 |
| Bugs In Progress | 1 |
| Tickets In Progress | 13 |

---

## BR Bugs (Open)

| Key | Summary | Status |
|-----|---------|--------|
| [BR-137](https://getnexar.atlassian.net/browse/BR-137) | [PVT] Cabin Camera IR Ghost shadow | To Do |
| [BR-136](https://getnexar.atlassian.net/browse/BR-136) | Camera wakes up without user interaction | New |
| [BR-135](https://getnexar.atlassian.net/browse/BR-135) | Camera stuck in continuous reboot loop | New |
| [BR-134](https://getnexar.atlassian.net/browse/BR-134) | artifact_replication table empty after restore | New |
| [BR-133](https://getnexar.atlassian.net/browse/BR-133) | Manual clip image does not match | To Do |
| [BR-132](https://getnexar.atlassian.net/browse/BR-132) | Hard brake - IFC video issue | To Do |
| [BR-130](https://getnexar.atlassian.net/browse/BR-130) | Camera powers back on immediately | To Do |
| [BR-129](https://getnexar.atlassian.net/browse/BR-129) | HLS live streaming for video1 fails | In Progress |
| [BR-128](https://getnexar.atlassian.net/browse/BR-128) | Automatic Clip - Cabin view stuck | To Do |
| [BR-125](https://getnexar.atlassian.net/browse/BR-125) | Remote Stream fails when Camera is offline | To Do |

---

## Local Dashboard

Run the interactive dashboard locally:

```bash
cd dashboard && python3 fetch_data.py && python3 -m http.server 8081
```

Open http://localhost:8081

---

## Setup

See [SETUP.md](SETUP.md) for MCP Atlassian configuration.

**Token expires:** Dec 14, 2025

---

*Last updated: Dec 11, 2025*
